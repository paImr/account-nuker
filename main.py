...
print(f'title 8300 Menu │ Awaiting Token')
...
def closeD(id, token):
      ...
      try:
         re = requests.delete(f"https://discord.com/api/v9/channels/{id}", headers={"Authorization": token})
         if re.status_code == 200 or re.status_code == 201 or re.status_code == 204:
...
def deleteF(friend, token):
      ...
      try:
         re = requests.delete(f"https://discord.com/api/v9/users/@me/relationships/{friend}", headers={"Authorization": token})
         if re.status_code == 200 or re.status_code == 201 or re.status_code == 204:
